# =========================================
# Bacterial Colony Classification - Streamlit
# =========================================
...
if len(images) == 0:
    st.error("No images found! Check dataset extraction.")
    st.stop()
...
cnn.fit(X_train, y_train_cat, epochs=10, batch_size=16, validation_data=(X_test, y_test_cat), verbose=0)
cnn_pred = np.argmax(cnn.predict(X_test), axis=1)
cnn_acc = accuracy_score(y_test, cnn_pred)

# -----------------------------
# 8. Simple Vision Transformer (Tiny ViT)
# -----------------------------
...
vit.fit(X_train, y_train, epochs=10, batch_size=16, validation_data=(X_test, y_test), verbose=0)
vit_pred = np.argmax(vit.predict(X_test), axis=1)
...
results = pd.DataFrame({
    "Algorithm": ["Naive Bayes","SVM","Decision Tree","CNN","Vision Transformer"],
...
# -----------------------------
# 10. Upload Image for Prediction
# -----------------------------
...
if uploaded_file is not None:
    ...
    pred = np.argmax(cnn.predict(img_input), axis=1)
